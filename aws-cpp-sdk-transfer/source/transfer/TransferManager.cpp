...
namespace Aws
{
    namespace Transfer
    {
        ...
        {
#ifdef _MSC_VER
            const auto streamToPut = std::make_shared<Aws::FStream>(Aws::Utils::StringUtils::ToWString(handle->GetTargetFilePath().c_str()).c_str(), std::ios_base::in | std::ios_base::binary);
            DoMultiPartUpload(streamToPut, handle);
#else
            const auto streamToPut = std::make_shared<Aws::FStream>(handle->GetTargetFilePath().c_str(), std::ios_base::in | std::ios_base::binary);
            DoMultiPartUpload(streamToPut, handle);
#endif
        }
...
#ifdef _MSC_VER
            const auto fileStream = std::make_shared<Aws::FStream>(Aws::Utils::StringUtils::ToWString(fileName.c_str()).c_str(), std::ios_base::in | std::ios_base::binary);
#else
			const auto fileStream = std::make_shared<Aws::FStream>(fileName.c_str(), std::ios_base::in | std::ios_base::binary);
#endif
